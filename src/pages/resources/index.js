import React, { useState, useEffect } from "react";
import { useHistory } from "react-router-dom";
import { useSelector, useDispatch } from "react-redux";
import _ from "lodash";
import FileViewer from "react-file-viewer";

import NewLessonPlan from "./NewLessonPlan";
import PayCard from "./PayCard";
import ReceiptCard from "./ReceiptCard";
import Box from "@material-ui/core/Box";
import { Header } from "../../components";
import "react-multi-carousel/lib/styles.css";
import Button from "@material-ui/core/Button";
import ResourceCard from "./ResourceCard";
import Container from "@material-ui/core/Container";
import Typography from "@material-ui/core/Typography";
import ResourceCardView from "./ResourceCardView";
import { Fonts, BgColor, Color } from "../../theme/index";
import { withStyles } from "@material-ui/core/styles";
import { TextField } from "@material-ui/core";
import SearchIcon from "@material-ui/icons/Search";
import { InputAdornment } from "@material-ui/core";
import { useTranslation } from 'react-i18next';
import {
  getResourceList,
  getOtherResourceList,
  seacrhResourcePlan,
} from "../../redux/actions/resourceActions";
import Const from '../../helpers/const'
import queryString from 'query-string';
const styles = (theme) => ({
  container: {
    alignSelf: "flex-end",
    paddingLeft: 250,
    paddingTop: 70,
    paddingBottom: 20,
  },
  root: {
    maxWidth: 345,
    marginTop: 10,
    marginRight: 10,
  },
  media: {
    height: 140,
  },
  blueButton: {
    color: "#fff",
    height: "40px",
    borderRadius: "5px",
    backgroundColor: BgColor.myBlue,
    padding: "0 20px",
    "&:hover": {
      color: Color.textBlue,
      border: "1px solid" + Color.textBlue,
    },
  },
  mainHeading: {
    fontSize: "20px",
    fontFamily: Fonts.Medium,
  },
  overlayWrapper: {
    position: "fixed",
    display: "flex",
    flexDirection: "row",
    justifyContent: "center",
    alignItems: "center",
    width: "100%",
    height: "100vh",
    zIndex: "2000",
    backgroundColor: "rgba(0, 0, 0, 0.5)",
  },
  TextFields: {
    width: "55%",
    margin: "8px",
  },
});

const Resources = (props) => {
  const authUser = useSelector((store) => store.auth.user);
  const history = useHistory();
  const dispatch = useDispatch();
  const { classes } = props;
  const [resourceid, setresourceid] = useState("")
  const [state, setState] = useState({
    openLessonPlan: false,
    addingNewResource: false,
    payCard: false,
    receiptCard: false,
  });
  const [is_descriptionpaid,setdescriptionpaid]=useState(false)
  useEffect(() => {
    dispatch(getResourceList());
    dispatch(getOtherResourceList());
    const value=queryString.parse(window.location.search, { ignoreQueryPrefix: true });
    if(value.id){
        setresourceid(value.id)
        // openLessonPlanFun(value.id,true)
    }
  }, [dispatch]);

  const resourceList = useSelector((store) => store.resource.list);
  const resourceByOthers = useSelector((store) => store.resource.listByOthers);

  const [search, setSearch] = useState("");

  const [resourceData, setResourceData] = useState(null);
  const [isOwnPlan, setIsOwnPlan] = useState(true);
  const [displaylimit, setdisplaylimit] = useState(12);
  const [my_limit, setmylimit] = useState(displaylimit);
  const [others_limit, setotherslimit] = useState(displaylimit);

  const handleSearch = (event) => {
    if (event.key === "Enter") {
      dispatch(seacrhResourcePlan(search)).then((res) => {
        console.log("res", res);
      });
    }
  };

  const open = (toBeOpened) => {
    setState({
      [toBeOpened]: !state[toBeOpened],
    });
  };

  const closeAddingNewResource = () => {
    setState({
      addingNewResource: !state.addingNewResource,
    });
  };
  const openLessonPlanFun = (id, ownLessonPlan) => {
    var p = null;
    if (ownLessonPlan) {
      p = _.find(resourceList, function (o) {
        return o.id == id;
      });
    } else {
      p = _.find(resourceByOthers, function (o) {
        return o.id == id;
      });
    }
    setResourceData(p);
    setIsOwnPlan(ownLessonPlan);
    setState({
      openLessonPlan: !state.openLessonPlan,
    });
    setresourceid("")
  };
  const openPayCard = () => {
    setState({
      payCard: !state.payCard,
      openLessonPlan: true,
    });
  };
  const openReceiptCard = () => {
    setState({
      receiptCard: !state.receiptCard,
    });
  };
  const loadmore =(type)=>{
    if(type=="my_resources"){
      var limit=my_limit + displaylimit
      setmylimit(limit)
    }
    else{
      var limit=others_limit + displaylimit
      setotherslimit(limit)
    }
  }
  const {t , i18n} = useTranslation()
  const images=[
    Const.BASE_URL+'public/images/resources/image1.jpg',
    Const.BASE_URL+'public/images/resources/image2.jpg',
    Const.BASE_URL+'public/images/resources/image3.jpg',
    Const.BASE_URL+'public/images/resources/image4.jpg',
    Const.BASE_URL+'public/images/resources/image5.jpg',
    Const.BASE_URL+'public/images/resources/image6.jpg',
    Const.BASE_URL+'public/images/resources/image7.jpg',
    Const.BASE_URL+'public/images/resources/image8.jpg',
    Const.BASE_URL+'public/images/resources/image9.jpg',
    
  ];
  var imagecount=0;
  return (
    <>
      <Header history={history} />

      {state.payCard ? (
        <Container maxWidth="xl" className={classes.overlayWrapper }>
          <PayCard
            plan={resourceData}
            authUser={authUser}
            openPayCard={openPayCard}
            openReceiptCard={openReceiptCard}
            setdescriptionpaid={setdescriptionpaid}
          />
        </Container>
      ) : (
        ""
      )}
      {state.receiptCard ? (
        <Container maxWidth="xl" className={classes.overlayWrapper}>
          <ReceiptCard openReceiptCard={openReceiptCard} />
        </Container>
      ) : (
        ""
      )}

      {state.addingNewResource ? (
        <Container maxWidth="xl" className={classes.overlayWrapper}>
          <NewLessonPlan closeAddingNewResource={closeAddingNewResource} />
        </Container>
      ) : (
        ""
      )}

      <Container maxWidth="xl" className={classes.container+ ' resources-main'}>
        {state.openLessonPlan ? (
          <Box flexWrap="wrap" p={2} m={0} bgcolor="background.paper">
            <ResourceCardView
              data={resourceData}
              openPayCard={openPayCard}
              openLessonPlanFun={openLessonPlanFun}
              authUser={authUser}
              isOwnPlan={isOwnPlan}
              is_descriptionpaid={is_descriptionpaid}
            />
          </Box>
        ) : (
          <>
            <Box flexWrap="wrap" p={2} m={0} bgcolor="background.paper">
              <Box
                display="flex"
                flexDirection="row"
                justifyContent="space-between"
              >
                <Typography
                  className={classes.mainHeading}
                  style={{ fontSize: "20px", fontFamily: "PoppinsMedium" }}
                >
                  {t("my_resources")}
                </Typography>
                <Button
                  className={classes.blueButton}
                  onClick={() => open("addingNewResource")}
                >
                  {t("add_resource")}
                </Button>
              </Box>

              {/* <FileViewer
                fileType={"pdf"}
                filePath={"https://api.irevu.org/public/images/1621168706.crime-and-punishment-british-english-teacher-ver2.pdf"}
              /> */}

              <Box
                display="flex"
                flexDirection="row"
                flexWrap="wrap"
                paddingTop={0}
                bgcolor="background.paper"
              >
                {Array.isArray(resourceList)
                  ? resourceList.slice(0,my_limit).map((value, i) => {
                    if(resourceid==value.id){
                      
                      
                      openLessonPlanFun(value.id,true)
                    }
                      var image=""
                      if(imagecount>images.length-1)
                      {
                        imagecount=0
                      }
                      image=images[imagecount]
                      imagecount++
                      return (
                        <ResourceCard
                          data={value}
                          key={i}
                          ownLessonPlan={true}
                          openLessonPlan={state.openLessonPlan}
                          openLessonPlanFun={openLessonPlanFun}
                          image={image}
                        />
                      );
                    })

                  : "No result found."}
              </Box>
            </Box>
            {Array.isArray(resourceList) && resourceList.length > my_limit ? 
              (<Box flexWrap="wrap" p={2} m={0} bgcolor="background.paper">
                <Button className={classes.blueButton} onClick={()=>loadmore('my_resources')}>
                    {t("load_more")}
                </Button>
              </Box>) : ('')}
            <Box flexWrap="wrap" p={2} m={0} bgcolor="background.paper">
              <Typography className={classes.mainHeading}>
                {t("resources_by_others")}
              </Typography>
              <Box>
                <TextField
                  label={t("search_paper")}
                  variant="outlined"
                  type="text"
                  name="search"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  onKeyDown={handleSearch}
                  className={classes.TextFields}
                  InputProps={{
                    startAdornment: (
                      <InputAdornment position="start">
                        <SearchIcon />
                      </InputAdornment>
                    ),
                  }}
                />
              </Box>

              <Box
                display="flex"
                flexDirection="row"
                flexWrap="wrap"
                paddingTop={0}
                bgcolor="background.paper"
              >
                {Array.isArray(resourceByOthers)
                  ? resourceByOthers.slice(0,others_limit).map((value, i) => {
                    if(resourceid==value.id){
                      openLessonPlanFun(value.id,false)
                    }
                    var image=""
                      if(imagecount>3)
                      {
                        imagecount=0
                      }
                      image=images[imagecount]
                      imagecount++
                      return (
                        <ResourceCard
                          data={value}
                          key={i}
                          ownLessonPlan={false}
                          openLessonPlan={state.openLessonPlan}
                          openLessonPlanFun={openLessonPlanFun}
                          image={image}
                        />
                      );
                    })
                  : "No plans found."}
              </Box>
              {Array.isArray(resourceByOthers) && resourceByOthers.length > others_limit ? 
              (<Box flexWrap="wrap" p={2} m={0} bgcolor="background.paper">
                <Button className={classes.blueButton} onClick={()=>loadmore('others_limit')}>
                    {t("load_more")}
                </Button>
              </Box>) : ('')}

            </Box>
          </>
        )}
      </Container>
    </>
  );
};

export default withStyles(styles, { withTheme: true })(Resources);
