...
const PayCardDiv = ({ plan, authUser, openPayCard, openReceiptCard ,setdescriptionpaid}) => {
  const styles = useStyles();
  ...
  const sampledownload =() =>{
    let downloadurl=Const.BASE_URL+"resource/sampledownload?filename="+plan.paper+"&page_limit_no=-1&pdftype=full&type=resource&module_id="+plan.id+"&to="+plan.created_by+"&from="+authUser.id

    if(plan.is_page_description=='1'){
      setdescriptionpaid(true)
      openPayCard();
    }
    else{
    ...
    let lessonData = {
      id:plan.created_by,
      for : 'student',
      message : authUser.username+' Your Resources downloaded successfully',
      receiverMsg : "Resource file downloaded"
    }
    socket.emit('send_notification',lessonData)
    if(plan.is_page_description === "0"){
              ...
              <CardContent>
                ...
                >
                  <Box>
                    ...
                    <Box className={styles.smallText}>Resource - {plan.title}</Box>
                    <Box
                      className={styles.smallLightText}
                      style={{ opacity: ".4" }}
                    >
...
export const TeamCardDemo = React.memo(function TeamCard(props) {
  return (
    <PayCardDiv
      ...
      setdescriptionpaid={props.setdescriptionpaid}
    />
  );
});
