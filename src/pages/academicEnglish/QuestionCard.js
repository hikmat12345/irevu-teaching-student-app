...
const RecipeReviewCard = ({ type, data, authUser, toggleEditQuestion }) => {
  ...
  const acceptQuestion = (id) => {
    ...
    dispatch( acceptForumQuestion({questionId: id,sellerId: sellerId,accepted: true}) ).then((res) => {
      ...
      if ( res.type == 'success') {
        ...
        history.push("/academic-feedback/"+id);
      } else {
        if (res.payload) {
  ...
  const passQuestion = (id) => {
    history.push("/academic-feedback/");
  }
    ...
    <>
      ...
      <Card className={classes.root }>
        ...
        <CardContent style={{ padding: "16px" }}>
          ...
          {t("privacy_policy") == "Privacy Policy" ? (
            <span className="rawHtml" dangerouslySetInnerHTML={{__html: data.title }} onClick={opendisplay ? () => openQuestion(data.id) : ''} style={{cursor:"pointer"}}/>
            ):(
            <span className="rawHtml" dangerouslySetInnerHTML={{__html: data.title_cn ? data.title_cn: data.title}} onClick={opendisplay ? () => openQuestion(data.id) : ''} style={{cursor:"pointer"}}/>
            )}
          </Typography>
        </CardContent>
        ...
        <CardActions disableSpacing>
          ...
          <CardHeader
          subheader={'Price: '+(data.price.indexOf('RMB') >= 0 ? deductplatfromFee(data.price) : deductplatfromFee(data.price)+' RMB')}
        />
          <IconButton
        ...
        <Collapse in={expanded && expandedvalue==data.id} timeout="auto" unmountOnExit>
          <CardContent>
            ...
            {/* <Typography paragraph component={'span'}> */}
              {/*<div className="rawHtml" dangerouslySetInnerHTML={{__html: data.description}} />*/}
            {/* </Typography> */}
          </CardContent>
        </Collapse>
      </Card>
